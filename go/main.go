...
func main() {
	...
	flag.BoolVar(&fastPublic, "fast-public", false, "optimize settings for public internet targets")
	flag.Parse()
	...
	if adaptive && !fastPublic {
		dialTimeout = estimateTimeout(ip, baseTimeout)
	}
	if fastPublic {
		if dialTimeout > 80*time.Millisecond {
	...
	// Spawn worker goroutines (limited by (possibly adjusted) workers flag)
	numWorkers := workers
	if numWorkers < 1 {
		numWorkers = 100
	}
	...
	// Fallback probe for common ports with a longer timeout to avoid false negatives
	if len(open) == 0 {
		common := []int{22, 80}
		fallbackTimeout := 1 * time.Second
		for _, p := range common {
			addr := net.JoinHostPort(ip, strconv.Itoa(p))
			conn, err := net.DialTimeout("tcp", addr, fallbackTimeout)
			if err == nil {
				...
				_ = conn.Close()
				open = append(open, Result{Port: p, Status: "open", Banner: banner})
			}
		}
	}
